...
const submit = () => {
    form.put(route('financial-years.update', props.financialYear.id));
};
</script>

<template>
    <Head :title="'ویرایش سال مالی: ' + financialYear.name" />
    <AuthenticatedLayout>
        <div class="mb-6">
            <h1 class="text-xl font-bold text-slate-800">ویرایش سال مالی: {{ financialYear.name }}</h1>
        </div>
        ...
        <div class="max-w-2xl">
            <div class="card">
                <form @submit.prevent="submit" class="space-y-4">
                    <div>
                        <label for="name" class="block font-medium text-sm text-gray-700">نام سال مالی</label>
