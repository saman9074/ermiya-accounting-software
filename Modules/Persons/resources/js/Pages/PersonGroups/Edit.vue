...
const form = useForm({
    name: props.personGroup.name,
    price_list_id: props.personGroup.price_list_id,
});

const submit = () => form.put(route('person-groups.update', props.personGroup.id));
</script>

<template>
    <Head :title="'ویرایش گروه: ' + personGroup.name" />
    <AuthenticatedLayout>
        <div class="mb-6"><h1 class="text-xl font-bold text-slate-800">ویرایش گروه: {{ personGroup.name }}</h1></div>
        <div class="card max-w-2xl">
            <form @submit.prevent="submit" class="p-6 space-y-4">
                <div>
                    <label for="name" class="block font-medium text-sm text-gray-700">نام گروه</label>
