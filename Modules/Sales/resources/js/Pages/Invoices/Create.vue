...
const { activeCurrency, formatCurrency, fromBaseCurrency } = useCurrency();
...
const isPersonSelected = computed(() => !!form.person_id);

const addItem = () => {
    if (selectedProduct.value) {
        let unitPriceInBase = selectedProduct.value.sale_price;
        ...
        if (selectedPerson.value && selectedPerson.value.group?.price_list_id && selectedProduct.value.product_prices) {
            ...
            if (specialPrice) {
                unitPriceInBase = specialPrice.price;
            }
        }
        ...
        form.items.push({
            ...
            quantity: 1,
            unit_price: fromBaseCurrency(unitPriceInBase), // <-- حالا این تابع وجود دارد
            discount_type: 'amount',
            discount_value: 0,
        });
...
const updateAllItemPrices = () => {
    form.items.forEach((item, index) => {
        ...
        if (product) {
            let newPriceInBase = product.sale_price;
            ...
            if (selectedPerson.value && selectedPerson.value.group?.price_list_id && product.product_prices) {
                ...
                if (specialPrice) {
                    newPriceInBase = specialPrice.price;
                }
            }
            form.items[index].unit_price = fromBaseCurrency(newPriceInBase);
        }
    });
};
                        ...
                        <form @submit.prevent="submit">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                                <div>
                                    ...
                                    <v-select dir="rtl" :options="persons" label="name" :reduce="person => person.id" v-model="form.person_id" placeholder="ابتدا مشتری را انتخاب کنید..." />
                                </div>
                                <div>
                            ...
                            <div class="flex items-end gap-4 border-t pt-4 mb-6">
                                <div class="flex-grow">
                                    ...
                                    <v-select dir="rtl" :options="products" label="name" v-model="selectedProduct" placeholder="جستجوی کالا..." :disabled="!isPersonSelected" />
                                </div>
                                <button @click="addItem" type="button" class="bg-green-500 text-white px-4 py-2 rounded-md" :disabled="!isPersonSelected || !selectedProduct">افزودن</button>
                            </div>
                            <div v-if="!isPersonSelected" class="text-center text-sm text-red-500 mb-4">
                                برای افزودن کالا، لطفا ابتدا یک مشتری را انتخاب کنید.
                            </div>
                            ...
                            <div class="overflow-x-auto">
                                <table class="min-w-full">
                                    ...
                                    <tr>
                                        ...
                                        <th class="py-2 px-4 text-right">قیمت واحد ({{ activeCurrency.display_name }})</th>
                                        <th class="py-2 px-4 text-right">جمع ({{ activeCurrency.display_name }})</th>
                                        <th class="py-2 px-4 text-right" style="width: 200px;">تخفیف</th>
                                        <th class="py-2 px-4 text-right">مبلغ تخفیف ({{ activeCurrency.display_name }})</th>
                                        <th class="py-2 px-4 text-right">جمع کل ({{ activeCurrency.display_name }})</th>
                                    ...
                                    <tr v-for="(item, index) in calculations.items" :key="index" class="border-b">
                                        ...
                                        <td class="py-2 px-4">{{ formatCurrency(item.subtotal) }}</td>
                                        <td class="py-2 px-4">
                                            <div class="flex items-center">
                                                ...
                                                <select v-model="form.items[index].discount_type" class="text-xs border-gray-300 rounded-md shadow-sm ml-1">
                                                    <option value="amount">{{ activeCurrency.display_name }}</option>
                                                    <option value="percentage">%</option>
                                                </select>
                                            </div>
                                        </td>
                                        <td class="py-2 px-4 text-red-500">({{ formatCurrency(item.discount_amount) }})</td>
                                        <td class="py-2 px-4 font-bold">{{ formatCurrency(item.total) }}</td>
                            ...
                            <div class="mt-6">
                                ...
                                <textarea id="notes" v-model="form.notes" rows="3" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"></textarea>
                            </div>
                            ...
                            <div class="mt-6 pt-6 border-t grid grid-cols-1 md:grid-cols-2">
                                ...
                                <div class="space-y-4">
                                    <div class="flex justify-between">
                                        <span>جمع آیتم‌ها ({{ activeCurrency.display_name }}):</span>
                                        <span class="font-semibold">{{ formatCurrency(calculations.subtotal) }}</span>
                                    </div>
                                    <div class="flex justify-between items-center">
                                        ...
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-red-500 w-24 text-center">({{ formatCurrency(calculations.overallDiscountAmount) }})</span>
                                        </div>
                                    </div>
                                    <div class="flex justify-between font-bold text-xl border-t pt-2">
                                        <span>مبلغ قابل پرداخت ({{ activeCurrency.display_name }}):</span>
                            ...
                            <div class="mt-8 flex justify-end">
                                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-md font-bold" :disabled="form.processing || form.items.length === 0">ثبت فاکتور</button>
                            </div>
                        </form>
                    </div>
                </div>
